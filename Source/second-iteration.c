        ...
        case ENTRY_LINE:
            ...
            if (fatalError) {
                ...
                if (entriesContent != NULL)
                    free(entriesContent);
                if (externalsContent != NULL)
                    free(externalsContent);
                return FALSE;
            }
            break;
    ...
    if ((tempSymbol = searchSymbol(*(context->symbolTable), arg)) != NULL) {
        if ((*tempSymbol).type == TYPE_EXTERNAL) {
            ...
            *errorFlagP = TRUE;
            ...
            writeToEntries(*tempSymbol, entriesContentP, entriesContentCapacityP, lineNum, errorFlagP, context);
        }
    } else {
...
void writeToEntries(const symbol symbol, char **entriesContentP, size_t *entriesContentCapacityP, const int lineNum,
    ...
    newCapacity =
        *entriesContentCapacityP + strlen(addressStr) + BASE4_ADDRESS_PADDING + strlen(symbol.label) + extraChars;
...
void handleExternal(const char *label, binaryWordNode *currentWordP, char **externalsContentP,
    ...
    if ((tempEntry = searchSymbol(*entriesTable, label)) != NULL) {
        ...
        writeToExternals(label, currentWordP, externalsContentP, externalsContentCapacityP, lineNum, errorFlagP,
                         context);
        if (fatalError)
            return;
    } else {
...
void writeToExternals(const char *label, const binaryWordNode *currentWordP, char **externalsContentP,
    ...
    newCapacity = *externalsContentCapacityP + strlen(addressStr) + BASE4_ADDRESS_PADDING + strlen(label) + extraChars;
...
void createObjectOutputFile(const char *fileName, const binaryWordList *codeImage, const int ICF, const int DCF) {
    FILE *objectFile;
    char *objectFileName;
    char *icfStr = intToBase4(ICF, COUNTER_REP, 0);
    ...
    objectFileName = malloc(strlen(fileName) + 4);
    ...
    while (currentWordP != NULL && i < ICF + DCF) {
        ...
        addressStr = intToBase4(INITIAL_ADDRESS + i, ADDRESS_REP, 0);
...
void createEntriesOutputFile(const char *fileName, const char *entriesContent) {
    FILE *entriesFile;
    char *entriesFileName = malloc(strlen(fileName) + 5);
...
void createExternalsOutputFile(const char *fileName, const char *externalsContent) {
    FILE *externalsFile;
    char *externalsFileName = malloc(strlen(fileName) + 5);
