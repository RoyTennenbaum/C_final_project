...
char *intToBase4(int integer) {
    ...
    int i = 0;
    printf("word value before converting to base 4:, %u", integer);
    ...
    printf("word value after converting to base 4:, %s", base4Address);
    return base4Address;
}
...
int handleTwoOperandOpEncoding(binaryWordNode *opFirstWordP, char *lineCopy, unsigned int srcOperandAddressEncoding,
    ...
    *commaPos = '\0';
    ...
    if (srcOperandAddressEncoding == MATRIX_OPCODE_ENCODING)
        currentWordP = ((opFirstWordP->next)->next)->next;
    else
